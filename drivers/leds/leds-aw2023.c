...
static int aw2023_write(struct aw2023* led, u8 reg, u8 val) {
...
static int aw2023_read(struct aw2023* led, u8 reg, u8* val) {
	int ret = -EINVAL, retry_times = 0;
...
static void aw2023_set_brightness(struct led_classdev* cdev, enum led_brightness brightness) {
	struct aw2023* led = container_of(cdev, struct aw2023, cdev);
...
static ssize_t aw2023_store_blink(struct device* dev, struct device_attribute* attr,
	...
	if (ret) return ret;
	mutex_lock(&led->pdata->led->lock);
	aw2023_blink_set(led, blinking);
	mutex_unlock(&led->pdata->led->lock);

	return len;
}

static ssize_t aw2023_time_show(struct device* dev, struct device_attribute* attr, char* buf) {
	struct led_classdev* led_cdev = dev_get_drvdata(dev);
	struct aw2023* led = container_of(led_cdev, struct aw2023, cdev);
...
static ssize_t aw2023_time_store(struct device* dev, struct device_attribute* attr,
									 const char* buf, size_t len) {
	struct led_classdev* led_cdev = dev_get_drvdata(dev);
	struct aw2023* led = container_of(led_cdev, struct aw2023, cdev);
	int rc, rise_time_ms, hold_time_ms, fall_time_ms, off_time_ms;

	rc = sscanf(buf, "%d %d %d %d", &rise_time_ms, &hold_time_ms, &fall_time_ms, &off_time_ms);

	mutex_lock(&led->pdata->led->lock);
	led->pdata->rise_time_ms = (rise_time_ms > MAX_RISE_TIME_MS) ? MAX_RISE_TIME_MS : rise_time_ms;
	led->pdata->hold_time_ms = (hold_time_ms > MAX_HOLD_TIME_MS) ? MAX_HOLD_TIME_MS : hold_time_ms;
	led->pdata->fall_time_ms = (fall_time_ms > MAX_FALL_TIME_MS) ? MAX_FALL_TIME_MS : fall_time_ms;
	led->pdata->off_time_ms = (off_time_ms > MAX_OFF_TIME_MS) ? MAX_OFF_TIME_MS : off_time_ms;
	aw2023_blink_set(led, 1);
	mutex_unlock(&led->pdata->led->lock);
	return len;
}

static ssize_t aw2023_reg_show(struct device* dev, struct device_attribute* attr, char* buf) {
	struct led_classdev* led_cdev = dev_get_drvdata(dev);
	struct aw2023* led = container_of(led_cdev, struct aw2023, cdev);
	...
	for (i = 0; i < AW2023_REG_MAX; i++) {
		if (!(aw2023_reg_access[i] & REG_RD_ACCESS)) continue;
...
static struct attribute_group aw2023_attr_group = {.attrs = aw2023_attributes};

static int aw2023_check_chipid(struct aw2023* led) {
	u8 val;
	u8 cnt;
	...
	for (cnt = 5; cnt > 0; cnt--) {
		...
		if (val == AW2023_CHIPID) return 0;
	}
	return -EINVAL;
}

static int aw2023_err_handle(struct aw2023* led_array, int parsed_leds) {
	int i;
	/*
	...
	for (i = 0; i < parsed_leds; i++) {
		sysfs_remove_group(&led_array[i].cdev.dev->kobj, &aw2023_attr_group);
...
free_class:
	aw2023_err_handle(led_array, parsed_leds);
...
static int aw2023_probe(struct i2c_client* client) {
	...
	if (!num_leds) return -EINVAL;

	led_array = devm_kzalloc(&client->dev, (sizeof(struct aw2023) * num_leds), GFP_KERNEL);
	...
	ret = aw2023_parse_child_node(led_array, node);
	if (ret) {
...
fail_parsed_node:
	aw2023_err_handle(led_array, num_leds);
free_led_arry:
...
static void aw2023_remove(struct i2c_client* client) {
	struct aw2023* led_array = i2c_get_clientdata(client);
...
static const struct i2c_device_id aw2023_id[] = {
    {"aw2023", 0},
    {},
};
...
MODULE_DEVICE_TABLE(i2c, aw2023_id);
...
static struct i2c_driver aw2023_driver = {
    ...
    .id_table = aw2023_id,
	.probe_new = aw2023_probe,
	.remove = aw2023_remove,
};

static int __init aw2023_init(void) {
	return i2c_add_driver(&aw2023_driver);
}
module_init(aw2023_init);

static void __exit aw2023_exit(void) {
	i2c_del_driver(&aw2023_driver);
}
module_exit(aw2023_exit);
