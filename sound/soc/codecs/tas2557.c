...
static void tas2557_fw_ready(const struct firmware *fw_entry, void *context)
{
	...
	for (i = 0; i < fw->num_programs; i++)
		dev_dbg(tas2557->dev, "  program[%u]: %s\n",
	...
	if (ret < 0)
		...
		dev_info(tas2557->dev,
			 "%s %s %u-bit ready: firmware '%s' v0x%x drv0x%x (%u programs, %u configs)\n",
			 tas2557_pg_name(tas2557->pg_id),
			 tas2557->channel ? "right" : "left", tas2557->i2s_bits,
			 tas2557->fw_name, fw->fw_version, fw->driver_version,
...
static void tas2557_irq_work_func(struct work_struct *work)
{
	...
	if ((int1 & 0xfc) || (int2 & 0x0c)) {
		...
		if (int2 & 0x08)
			tas2557->err_code |= ERROR_CLK_DET1;
		if (int2 & 0x04)
			tas2557->err_code |= ERROR_CLK_DET2;
		goto reset;
	}
...
static int tas2557_enable(struct tas2557_priv *tas2557, bool enable)
{
	...
	if (enable && !tas2557->powered) {
		 ...
		 * Playback runs over ASI2; ASI1 is set too, matching the
		 * TI reference offset programming.
		 */
		...
		/* Firmware + config are live: arm the fault IRQ */
		...
		/* Mask the fault IRQ before powering the Class-D down */
		if (tas2557->irq && tas2557->irq_enabled) {
			     ...
			     struct snd_soc_dai *dai)
{
	...
	/*
	 * Channel data-slot offset in BCLK cycles (not bytes): the left
	 * channel sits at offset 1, the right channel one sample behind
	 * it (1 + i2s_bits*2), matching the TI reference driver.  Cache it
	 * so tas2557_enable() can re-apply it after the SW reset in
	 * tas2557_set_program() clears the ASI offset registers.
	 */
	offset = (tas2557->channel == 0) ? 1 : (1 + tas2557->i2s_bits * 2);
	...
	dev_dbg(tas2557->dev, "ASI: %u-bit, %s channel, offset=%u BCLK cycles\n",
		tas2557->i2s_bits,
...
static int tas2557_i2c_probe(struct i2c_client *client)
{
	...
	/* Seed cached ASI offset (BCLK cycles) from the DT channel/width */
	tas2557->asi_offset = (tas2557->channel == 0) ? 1 :
			      (1 + tas2557->i2s_bits * 2);
	...
	/* 1.8 V supplies (AVDD, DVDD, IOVDD); often a single shared rail */
	tas2557->supplies[0].supply = "iovdd";
	tas2557->supplies[1].supply = "avdd";
	tas2557->supplies[2].supply = "dvdd";
	ret = devm_regulator_bulk_get(dev, ARRAY_SIZE(tas2557->supplies),
				      tas2557->supplies);
	if (ret)
		return dev_err_probe(dev, ret, "failed to get supplies\n");

	ret = regulator_bulk_enable(ARRAY_SIZE(tas2557->supplies),
				    tas2557->supplies);
	if (ret)
	...
	dev_dbg(dev, "TAS2557 probed (I2C addr 0x%02x, %s channel, %u-bit I2S)\n",
		client->addr, tas2557->channel ? "right" : "left",
		tas2557->i2s_bits);
	return 0;

err_disable_reg:
	regulator_bulk_disable(ARRAY_SIZE(tas2557->supplies), tas2557->supplies);
	return ret;
}
...
static void tas2557_i2c_remove(struct i2c_client *client)
{
	...
	/* Orderly Class-D shutdown before tearing the device down */
	if (tas2557->powered)
		tas2557_enable(tas2557, false);
