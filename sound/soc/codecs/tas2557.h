...
/*
 * YRAM address ranges used by the firmware YRAM CRC8 verification.
 * Values are identical to the TI reference driver (tas2557-core.h).
 */
...
struct tas2557_priv {
	...
	/*
	 * Cached ASI data-slot offset in BCLK cycles (left/ch0 = 1,
	 * right/ch1 = 1 + i2s_bits*2).  Computed in hw_params/set_tdm_slot
	 * and re-applied in tas2557_enable() after the SW reset in
