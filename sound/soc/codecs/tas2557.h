 ...
 * - 256 books (0-255)
 * - 256 pages per book (0-255)
 * - 128 registers per page (0-127)
 *
 * Address encoding: 0x00BBPPPR where BB=book, PP=page, R=register
 */
...
/* Book 0, Page 1 registers - ASI configuration */
 ...
 *
 * TAS2557 firmware format (from TI PPC tool):
 * - Header with magic number, version info, device info
 * - PLL configurations
 * - DSP programs
 * - Audio configurations (sample rates, etc.)
 */

/* Firmware magic number: "5552" in ASCII */
...
/* Generic data container with blocks */
struct tas2557_data {
	char name[64];
	char *description;
	u32 num_blocks;
...
#define TAS2557_TEMP_CHECK_PERIOD	1000

/* Die temperature thresholds */
#define TAS2557_SAFE_TEMP_LOW		-40
#define TAS2557_SAFE_TEMP_HIGH		150
